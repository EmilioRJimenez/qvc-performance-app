...
$('#example2').DataTable({        
	...
	function sacarTiempo(){
		...
		var calidad = $('#calidad').val(); calidad = Number(calidad);
		var mantto = $('#mantto').val(); mantto = Number(mantto); 
		var materiales = $('#materiales').val(); materiales = Number(materiales);
		var cdd = $('#cdd').val(); cdd = Number(cdd);
		var procesos = $('#procesos').val(); procesos = Number(procesos);
		var enrredos = $('#enrredos').val(); enrredos = Number(enrredos);
		var atorones = $('#atorones').val(); atorones = Number(atorones);
		var sello = $('#sello').val(); sello = Number(sello);
		var setupa = $('#setupa').val(); setupa = Number(setupa);
		var setupb = $('#setupb').val(); setupb = Number(setupb);
		var setupc = $('#setupc').val(); setupc = Number(setupc);
		var setupd = $('#setupd').val(); setupd = Number(setupd);
		var ajuste = $('#ajuste').val(); ajuste = Number(ajuste);
		var otrostiempos = $('#otrosTiempos').val(); otrostiempos = Number(otrostiempos);



		var tiempoDisponible = $('#tiemposDisponible').val();
		console.log(tiempoDisponible);
		...
		totalTiempo = calidad+mantto+materiales+cdd+procesos+enrredos+atorones+sello+setupa+setupb+setupc+setupd+ajuste+otrostiempos;
	...
	function otrosInteraction(){
		var errores = $('#errores').val(); errores = Number(errores);
		var defectos = $('#defectos').val(); defectos = Number(defectos);
	...
	function calidadIntercation(){
			...
			$.ajax({
				...
				success: function(datos){
					...
					datos.forEach(element => {
						$('#opcion').append(`<option class='op' value='${element.nombre}'>${element.nombre}</option>`);
					})
				}
			})
		}
		...
		$('#btnGenerarGrafica').click(function(e){
			...
			let formData = {
				reporte, turno, tipo, opcion, fechaInicio, fechaFin
			}
			...
			$.ajax({
				...
				$('#produccionEfectividad').text(Math.round(porcentajeEfectividad) + '%');
